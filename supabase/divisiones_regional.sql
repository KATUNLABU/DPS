-- ====================================================================
-- KATUN REGIONAL DATABASE SCHEMA (DPS_MX, DPS_BR, KWS_MX, KWS_BR)
-- Copia y ejecuta este script en el Editor SQL de Supabase (SQL Editor)
-- ====================================================================

-- --------------------------------------------------------------------
-- 1. ESTRUCTURA Y POBLACIÓN DE DPS_MX (PROYECTO ACTUAL KATUN DPS MÉXICO)
-- --------------------------------------------------------------------

-- TABLA DPS_MX: INDUSTRIAS
CREATE TABLE IF NOT EXISTS public.dps_mx_industries (
...
-- TABLA DPS_MX: PRODUCTOS (LOS 14 EQUIPOS)
CREATE TABLE IF NOT EXISTS public.dps_mx_products (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    slug TEXT UNIQUE NOT NULL,
    name TEXT NOT NULL,
...
-- TABLA DPS_MX: CATÁLOGOS
CREATE TABLE IF NOT EXISTS public.dps_mx_catalogs (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    title TEXT NOT NULL,
    description TEXT NOT NULL,
    pdf_url TEXT NOT NULL,
    category TEXT DEFAULT 'General',
    product_slug TEXT REFERENCES public.dps_mx_products(slug) ON DELETE CASCADE,
    created_at TIMESTAMPTZ DEFAULT NOW()
);

-- TABLA DPS_MX: RELACIÓN PRODUCTO <-> INDUSTRIA
CREATE TABLE IF NOT EXISTS public.dps_mx_product_industries (
    product_id UUID REFERENCES public.dps_mx_products(id) ON DELETE CASCADE,
    industry_id UUID REFERENCES public.dps_mx_industries(id) ON DELETE CASCADE,
    PRIMARY KEY (product_id, industry_id)
);

-- TABLA DPS_MX: REGISTROS Y FORMULARIOS DE LA WEB
CREATE TABLE IF NOT EXISTS public.dps_mx_registros (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
-- --------------------------------------------------------------------
-- 2. ESTRUCTURA VACÍA PARA DIVISIÓN: DPS_BR (DPS BRASIL)
-- --------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.dps_br_industries (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    slug TEXT UNIQUE NOT NULL,
    title TEXT NOT NULL,
    description TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.dps_br_products (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    slug TEXT UNIQUE NOT NULL,
    name TEXT NOT NULL,
    description TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.dps_br_registros (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    full_name TEXT NOT NULL,
    email TEXT NOT NULL,
    created_at TIMESTAMPTZ DEFAULT NOW()
...
-- --------------------------------------------------------------------
-- 3. ESTRUCTURA VACÍA PARA DIVISIÓN: KWS_MX (KATUN WEB SOLUTIONS MÉXICO)
-- --------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.kws_mx_industries (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    slug TEXT UNIQUE NOT NULL,
    title TEXT NOT NULL,
    description TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.kws_mx_products (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    slug TEXT UNIQUE NOT NULL,
    name TEXT NOT NULL,
    description TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.kws_mx_registros (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    full_name TEXT NOT NULL,
    email TEXT NOT NULL,
    created_at TIMESTAMPTZ DEFAULT NOW()
...
-- --------------------------------------------------------------------
-- 4. ESTRUCTURA VACÍA PARA DIVISIÓN: KWS_BR (KATUN WEB SOLUTIONS BRASIL)
-- --------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.kws_br_industries (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    slug TEXT UNIQUE NOT NULL,
    title TEXT NOT NULL,
    description TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.kws_br_products (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    slug TEXT UNIQUE NOT NULL,
    name TEXT NOT NULL,
    description TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.kws_br_registros (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    full_name TEXT NOT NULL,
    email TEXT NOT NULL,
    created_at TIMESTAMPTZ DEFAULT NOW()
...
-- --------------------------------------------------------------------
-- 5. SEGURIDAD (RLS) PARA DPS_MX
-- --------------------------------------------------------------------
ALTER TABLE public.dps_mx_industries ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.dps_mx_products ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.dps_mx_catalogs ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.dps_mx_product_industries ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.dps_mx_registros ENABLE ROW LEVEL SECURITY;

ALTER TABLE public.dps_br_industries ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.dps_br_products ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.dps_br_registros ENABLE ROW LEVEL SECURITY;

ALTER TABLE public.kws_mx_industries ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.kws_mx_products ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.kws_mx_registros ENABLE ROW LEVEL SECURITY;

ALTER TABLE public.kws_br_industries ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.kws_br_products ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.kws_br_registros ENABLE ROW LEVEL SECURITY;

-- POLÍTICAS DE ACCESO DPS_MX
DROP POLICY IF EXISTS "Lectura pública de DPS MX industrias" ON public.dps_mx_industries;
DROP POLICY IF EXISTS "Permitir inserción de DPS MX industrias" ON public.dps_mx_industries;
DROP POLICY IF EXISTS "Lectura pública de DPS MX productos" ON public.dps_mx_products;
DROP POLICY IF EXISTS "Permitir inserción de DPS MX productos" ON public.dps_mx_products;
DROP POLICY IF EXISTS "Permitir envío público de DPS MX registros" ON public.dps_mx_registros;
DROP POLICY IF EXISTS "Lectura privada de DPS MX registros" ON public.dps_mx_registros;

CREATE POLICY "Lectura pública de DPS MX industrias" ON public.dps_mx_industries FOR SELECT USING (true);
CREATE POLICY "Permitir inserción de DPS MX industrias" ON public.dps_mx_industries FOR INSERT WITH CHECK (true);
CREATE POLICY "Lectura pública de DPS MX productos" ON public.dps_mx_products FOR SELECT USING (true);
CREATE POLICY "Permitir inserción de DPS MX productos" ON public.dps_mx_products FOR INSERT WITH CHECK (true);
CREATE POLICY "Permitir envío público de DPS MX registros" ON public.dps_mx_registros FOR INSERT WITH CHECK (true);
CREATE POLICY "Lectura privada de DPS MX registros" ON public.dps_mx_registros FOR SELECT USING (auth.role() = 'authenticated' OR auth.role() = 'service_role');

-- POLÍTICAS GENERALES PARA TABLAS VACÍAS
CREATE POLICY "Lectura pública DPS BR products" ON public.dps_br_products FOR SELECT USING (true);
CREATE POLICY "Lectura pública KWS MX products" ON public.kws_mx_products FOR SELECT USING (true);
CREATE POLICY "Lectura pública KWS BR products" ON public.kws_br_products FOR SELECT USING (true);

-- --------------------------------------------------------------------
-- 6. POBLACIÓN COMPLETA DE DATOS EN DPS_MX
-- --------------------------------------------------------------------
INSERT INTO public.dps_mx_industries (slug, title, tag, icon, image, description, subsections)
VALUES
(
    'bebidas',
...
INSERT INTO public.dps_mx_products (slug, name, subtitle, category, technology, speed, resolution, max_width, description, features, specs, pdf_url, badge, image)
VALUES
(
    'arrowjet-aqua-330r',
    'ArrowJet Aqua 330R',
