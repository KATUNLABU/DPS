-- ====================================================================
-- KATUN - ESTRUCTURA FINAL DPS_MX (SUBDIVIDIDA) + (DPS_BR, KWS_MX, KWS_BR)
-- Copia y ejecuta este script en tu Editor SQL de Supabase (SQL Editor)
-- ====================================================================

-- 1. LIMPIEZA DE TABLAS PREVIAS DE PRUEBA
...
-- ====================================================================
-- 2. TABLAS SUBDIVIDIDAS PARA LA DIVISIÓN "DPS_MX" (PROYECTO ACTUAL)
-- ====================================================================

-- TABLA DPS_MX: INDUSTRIAS / SECTORES
CREATE TABLE public.dps_mx_industries (
...
-- TABLA DPS_MX: PRODUCTOS / MÁQUINAS (LOS 14 EQUIPOS)
CREATE TABLE public.dps_mx_products (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    slug TEXT UNIQUE NOT NULL,
    name TEXT NOT NULL,
...
-- TABLA DPS_MX: CATÁLOGOS PDF
CREATE TABLE public.dps_mx_catalogs (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    title TEXT NOT NULL,
    description TEXT NOT NULL,
...
CREATE TABLE public.dps_mx_product_industries (
...
-- TABLA DPS_MX: REGISTROS Y FORMULARIOS UNIFICADOS DE LA WEB
CREATE TABLE public.dps_mx_registros (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
-- ====================================================================
-- 3. TABLAS MAESTRAS VACÍAS PARA LAS OTRAS 3 DIVISIONES
-- ====================================================================
...
CREATE POLICY "Lectura pública dps_mx_industries" ON public.dps_mx_industries FOR SELECT USING (true);
CREATE POLICY "Lectura pública dps_mx_products" ON public.dps_mx_products FOR SELECT USING (true);
CREATE POLICY "Lectura pública dps_mx_catalogs" ON public.dps_mx_catalogs FOR SELECT USING (true);
CREATE POLICY "Lectura pública dps_mx_product_industries" ON public.dps_mx_product_industries FOR SELECT USING (true);

CREATE POLICY "Permitir envío público dps_mx_registros" ON public.dps_mx_registros FOR INSERT WITH CHECK (true);
CREATE POLICY "Lectura privada dps_mx_registros" ON public.dps_mx_registros FOR SELECT USING (auth.role() = 'authenticated' OR auth.role() = 'service_role');
CREATE POLICY "Actualización dps_mx_registros" ON public.dps_mx_registros FOR UPDATE USING (auth.role() = 'authenticated' OR auth.role() = 'service_role');

CREATE POLICY "Lectura dps_br" ON public.dps_br FOR SELECT USING (true);
CREATE POLICY "Lectura kws_mx" ON public.kws_mx FOR SELECT USING (true);
CREATE POLICY "Lectura kws_br" ON public.kws_br FOR SELECT USING (true);

-- Índices de rendimiento
CREATE INDEX idx_dps_mx_registros_email ON public.dps_mx_registros(email);
CREATE INDEX idx_dps_mx_registros_status ON public.dps_mx_registros(status);
CREATE INDEX idx_dps_mx_registros_created ON public.dps_mx_registros(created_at DESC);

-- ====================================================================
-- 5. POBLAR DPS_MX CON LOS DATOS DE ESTA PÁGINA WEB
-- ====================================================================
